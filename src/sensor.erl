    ...
    case HasDirectServerConnection of
        true ->
            case whereis(server) of
                undefined ->                
                case rpc:call('server@new-host-4', erlang, whereis, [server], 1000) of
                    {badrpc, _} -> undefined;
                    undefined -> undefined;
    ...
    case whereis(SensorName) of
        ...
        Pid -> 
            ...
            case find_neighbor(Neighbor) of
                undefined -> 
                    io:format("[~p] Não foi possível encontrar sensor ~p para adicionar conexão bidirecional~n", [SensorName, Neighbor]);
                NeighborPid -> 
                    NeighborPid ! {add_neighbor_silent, SensorName},
                    io:format("[~p] Conexão bidirecional estabelecida com ~p~n", [SensorName, Neighbor])
            end,
            ok
    end.
...
remove_neighbor(SensorName, Neighbor) ->
    case whereis(SensorName) of
        ...
        Pid -> 
            ...
            case find_neighbor(Neighbor) of
                undefined -> 
                    io:format("[~p] Não foi possível encontrar sensor ~p para remover conexão bidirecional~n", [SensorName, Neighbor]);
                NeighborPid -> 
                    NeighborPid ! {remove_neighbor_silent, SensorName},
                    io:format("[~p] Conexão bidirecional removida com ~p~n", [SensorName, Neighbor])
            end,
            ok
    end.
...
notify_neighbors_of_death(Name, Neighbors) ->
    ...
    lists:foreach(fun(Neighbor) ->
        case find_neighbor(Neighbor) of
            undefined -> 
                io:format("[~p] Não foi possível encontrar vizinho ~p para notificar morte~n", [Name, Neighbor]);
            NeighborPid -> 
...
notify_neighbors_of_birth(Name, Neighbors) ->
    ...
    lists:foreach(fun(Neighbor) ->
        case find_neighbor(Neighbor) of
            undefined -> 
                io:format("[~p] Não foi possível encontrar vizinho ~p para notificar nascimento~n", [Name, Neighbor]);
            NeighborPid -> 
    ...
    receive
        ...
        {add_neighbor, Neighbor} ->
            UpdatedNeighbors = case lists:member(Neighbor, Neighbors) of
                true -> 
                    io:format("[~p] Vizinho ~p já existe na lista~n", [Name, Neighbor]),
                    Neighbors;
                false -> 
        ...
        {remove_neighbor, Neighbor} ->
            UpdatedNeighbors = case lists:member(Neighbor, Neighbors) of
                false -> 
                    io:format("[~p] Vizinho ~p não existe na lista~n", [Name, Neighbor]),
                    Neighbors;
                true -> 
...
find_neighbor(NeighborName) ->
    ...
    case whereis(NeighborName) of
        undefined ->
            ...
            NodeName = list_to_atom(atom_to_list(NeighborName) ++ "@new-host-4"),
            case rpc:call(NodeName, erlang, whereis, [NeighborName], 1000) of
                {badrpc, Reason} -> 
                    io:format("Erro RPC ao contactar nó ~p: ~p~n", [NodeName, Reason]),
                    undefined;
                undefined -> 
                    io:format("Vizinho ~p não encontrado no nó ~p~n", [NeighborName, NodeName]),
                    undefined;
                RemotePid -> 
...
try_relay_all([], _, FailedNeighbors) ->
    case FailedNeighbors of
        [] -> {error, no_neighbors};
        _ -> 
            io:format("Todos os vizinhos falharam: ~p~n", [FailedNeighbors]),
            {error, no_path}
    end;
try_relay_all([N|Ns], Msg, FailedNeighbors) ->
    case find_neighbor(N) of
        undefined -> 
            io:format("Vizinho ~p não encontrado (possivelmente morreu), tentando próximo...~n", [N]),
...
handle_relay(Name, Neighbors, Msg) ->
    ...
    case ServerPid of
        undefined -> 
            io:format("[~p] Não tenho conexão direta, reenviando para vizinhos ~p~n", [Name, Neighbors]),
    ...
    case lists:member(Name, Path) of
        ...
        false ->
            ...
            case ServerPid of
                undefined -> 
                    NewPath = [Name | Path],
                    io:format("[~p] Não tenho conexão direta, reenviando para vizinhos ~p (caminho: ~p)~n", [Name, Neighbors, NewPath]),
...
try_relay_with_path_all([N|Ns], Msg, Path, FailedNeighbors) ->
    ...
    case lists:member(N, Path) of
        true ->
            io:format("Vizinho ~p já está no caminho ~p, ignorando para evitar loop~n", [N, Path]),
